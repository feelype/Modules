...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
	...
	if (list_empty(&mylinkedlist))
	{
		
		
		struct structname *first_ponteiro;
		first_ponteiro=list_entry(&mylinkedlist, struct structname, mylist);
		error_count = copy_to_user(buffer, first_ponteiro, size_of_message);
		
		if (error_count==0){            // if true then have success
		printk(KERN_INFO "Simple Driver: sent %d characters to the user\n", size_of_message);
		printk(KERN_INFO "Simple Driver: sent %s characters to the user\n", first_ponteiro->words); //arrumar o print
		
		list_del(&first_ponteiro->mylist);
		kfree(first_ponteiro);
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
	if (len < sizeof(message)){
		...
		struct structname *tempNode;
		
		tempNode = kmalloc(sizeof(struct structname), GFP_KERNEL);
		
		list_add(&tempNode->mylist, &mylinkedlist);
		
		size_t ret  = copy_from_user(tempNode->words, buffer, len);
